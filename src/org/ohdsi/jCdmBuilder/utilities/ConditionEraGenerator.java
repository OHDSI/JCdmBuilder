...
/**
 * Creates condition_eras based on condition occurrences
 * 
 * @author MSCHUEMI
 */
public class ConditionEraGenerator {
	...
	public void addCondition(long personId, long startDate, long endDate, long conceptId) {
		if (startDate != StringUtilities.MISSING_DATE && conceptId != 0)
	...
	public List<Row> generateRows() {
		...
		Collections.sort(conditions, new Comparator<ConditionOccurence>() {
			...
			public int compare(ConditionOccurence o1, ConditionOccurence o2) {
				int result = LongComparator.compare(o1.personId, o2.personId);
				if (result == 0)
		...
		long oldPersonId = Integer.MIN_VALUE;
	...
	private void processPerson(List<ConditionOccurence> personExposures, List<Row> rows) {
		Map<Long, ConditionOccurence> conceptIdToLastExposure = new HashMap<Long, ConditionOccurence>();
	...
	private class ConditionOccurence {
		...
		public ConditionOccurence(long personId, long startDate, long endDate, long conceptId) {
